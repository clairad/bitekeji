...
void SeqListBubbleSort(SeqList* psl)
{
	...
	for (i = 0; i < psl->size - 1; i++)
	{
		...
		{
			if (Contactcmp(*psl->array[j], *psl->array[j + 1]) > 0)
			{
				tmp = psl->array[j];
...
int SeqListBinaryFind(SeqList* psl, SLDataType x)
{
	...
	{
		mid = (start + end) / 2;
		if (Contactcmp(*psl->array[mid], *x) < 0)
		{
			start = mid + 1;
		}
		else if (Contactcmp(*psl->array[mid], *x) > 0)
		{
			end = mid - 1;
		}
		else
